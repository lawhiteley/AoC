defmodule Day001 do
  ...
  def count_of_clicks(instructions, start \\ 50), do: elem(Enum.reduce(instructions, {start, 0}, &rotate(&1, &2)), 1)

  defp rotate("L" <> amount, {location, clicks}) do rotate(String.to_integer(amount) * -1, {location, clicks}) end
  defp rotate("R" <> amount, {location, clicks}) do rotate(String.to_integer(amount), {location, clicks}) end
